...
`mitos-utils` is a coreutils-style suite of ~64 small userspace
...
plus ~64 symlinks pointing at it, instead of ~64 separate copies of
...
applet at all, so this is handled once instead of ~64 times. It
...
to ~64 utilities: any fix to how errors are reported, or any POSIX
...
`mitos-utils` itself or of any of the ~64 shipped binaries. A plain
