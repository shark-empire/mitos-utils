...
`mitos-utils` is a coreutils-style suite of ~58 small userspace
...
plus ~58 symlinks pointing at it, instead of ~58 separate copies of
...
applet at all, so this is handled once instead of ~58 times. It
...
to ~58 utilities: any fix to how errors are reported, or any POSIX
...
`mitos-utils` itself or of any of the ~58 shipped binaries. A plain
